...
object Home {
    fun render(page: PageBuilder, state: LauncherUiState, actions: ScreenActions) {
        ...
        page.hero("StudioDroid v2", "2.0.0-m1 / LAUNCHER FOUNDATION", Badge("LAUNCH UNAVAILABLE"), "Studio · not installed")
        ...
        page.stage("host", "Android host", "", R.drawable.ic_cpu, host)
        val stages = listOf(
        ...
        stages.forEachIndexed { index, (kind, title, description) ->
            ...
            page.stage("ready-$kind", title, "", if (kind == ComponentKind.STUDIO) R.drawable.ic_studio else R.drawable.ic_runtime, componentBadge(record), index == stages.lastIndex, record?.identity?.version)
        }
